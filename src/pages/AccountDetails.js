...
const fetchAccountDetails = async () => {
  try {
    ...
    const response = await withAuthFetch(url, { method: SummaryApi.getAccountById.method });
...
const fetchEntries = async () => {
  try {
    ...
    const response = await withAuthFetch(url, { method: SummaryApi.getEntriesByAccount.method });
    if (response?.success) {
...
const handleAddEntry = async (entryData) => {
  try {
    ...
    return { success: false, message: response?.message || 'Failed to add entry' };
  } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b border-gray-100 px-4 py-3">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-lg font-semibold text-gray-900">Account Details</h1>
        </div>
      </div>
      ...
      {account && (
        <div className="bg-white mx-4 mt-4 p-6 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center space-x-4 mb-4">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-semibold text-xl ${getAvatarColor(account.accountName)}`}>
          ...
          <div className="text-center py-4 border-t border-gray-100">
            ...
            <p className={`text-3xl font-bold ${getBalanceColor(account.balance || 0)}`}>
